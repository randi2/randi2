...
public class BenutzerServlet extends javax.servlet.http.HttpServlet {
	...
	else if (id.equals(BenutzerServlet.anfrage_id.CLASS_DISPATCHERSERVLET_BENUTZER_REGISTRIEREN_VIER.name())) {
     ...
     *      HttpServletResponse response)
     */
    private void weiterleitungLoginKorrektBeiSystemGesperrt(BenutzerkontoBean aBenutzer, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

	Logger.getLogger(this.getClass()).debug("System gesperrt, Kandidat == Sysop?");
	if (aBenutzer.getRolle().getRollenname() == Rolle.Rollen.SYSOP) {
	    ...
	    request.setAttribute(DispatcherServlet.FEHLERNACHRICHT, " Das Systen ist derzeit gesperrt.<br> Login nicht m&ouml;glich!");
	    request.getRequestDispatcher("/index_gesperrt.jsp").forward(request, response);
	    // Ergebnis loggen
	    Logger.getLogger(this.getClass()).debug("System gesperrt, Benutzer abgewiesen");
	    LogAktion a = new LogAktion("Benutzer abgewiesen, System gesperrt", aBenutzer);
     ...
     *      HttpServletResponse response)
     */
    private void weiterleitungLoginKorrekt(BenutzerkontoBean aBenutzer, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	Logger.getLogger(this.getClass()).debug("BenutzerServlet.weiterleitungLoginKorrekt()");
	...
	if (aBenutzer.getRolle().getRollenname() == Rolle.Rollen.STUDIENARZT) {
	    ...
	    request.getRequestDispatcher("/systemadministration.jsp").forward(request, response);
	    loggeKorrekteanmeldung(aBenutzer);
	    return;
	}
     ...
     * </p>
     * 
     * @param fehlermeldungAnBenutzer Fehlermeldung, die dem Benutzer angzeigt wird
     * @param request
     *            Der Request fuer das Servlet.
     * @param response
     *            Der Response Servlet.
     ...
     *      HttpServletResponse response)
     */
    private void weiterleitungBeiFehler(String fehlermeldungAnBenutzer, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    *      HttpServletResponse response)
    */
    private void classDispatcherservletLogin1(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	...
	try {
		...
		if (!aBenutzer.isGesperrt() && new Benutzerkonto(aBenutzer).pruefenPasswort((String) request.getParameter("password"))) {
			...
			// Konto gesperrt
			a = new LogAktion("Versuchter Zugriff auf gesperrtes Konto", aBenutzer);
		    } else {
			a = new LogAktion("Falsches Passwort eingegeben.", aBenutzer);
		    }
		...
		if (gBenutzer.size() == 0) {
		    // Unbekanntes Konto
		    LogAktion a = new LogAktion("Unbekannter Benutzername.", sBenutzer);
		    Logger.getLogger(LogLayout.LOGIN_LOGOUT).warn(a);
		} else { // 
    ...
    /**
     * Realisiert den letzten Schritt des Benutzerregistierens. Die Eingabe der Personendaten
    * @param request
    *            Der Request fuer das Servlet.
    * @param response
    *            Der Response Servlet.
